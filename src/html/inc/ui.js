...
var renewalUI = { 
    ...
    openPopup: function(param) {
        ...
        if (typeof(param) == 'object') {
            ...
            cancelCallback = param.cancelCallback;  // 팝업의 cancel 버튼 실행 callback
													// 함수
            etcCallback = param.etcCallback;        // 팝업의 line 버튼 실행 callback
													// 함수
        }
    ...
    // voidAccordion (기능만 있음, 디자인 없음) : 추가 2023-11-03 김대화
    voidAccordion: function() {
        ...
        const toggle = function(val, idx) {
            ...
            for (const item of val) {
                ...
                let ph;
                // tab 안에서 display none 높이 구하기
                setTimeout(() => {
                    ph = panel.clientHeight;
        ...
        if (accordion.length >= 1) {
            ...
            for (const item of accordion) {
                ...
                // tab 안에서 display none 높이 구하기
                const tab = item.closest(".tab_panel:not(.active)");
                if (tab) {
